...

{
  ...
  programs.bash = {
    ...
    shellAliases = {
      ...
      nd = "diff /etc/nixos/configuration.nix $HOME/config/nix/configuration.nix";
      ns = "sudo nix-channel --update;sudo nixos-rebuild switch";
      nu = "cp /etc/nixos/configuration.nix $HOME/config/nix/configuration.nix";
      j = "jobs";
      k = "kubectl";
